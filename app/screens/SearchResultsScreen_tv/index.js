...
import SerieListRow from '../../components/cards/rows/SerieListRow';
import SerieRow from '../../components/cards/rows/SerieRow';
...
export default class SearchResultsScreen extends Component {
  ...
  requestMoviesList = async () => {
    try {
      ...
      const query = typeRequest === 'search'
          ? { query: `${name.trim()}` }
          : { with_genres: `${id}` };

      const data = await request(`${typeRequest}/tv`, {
        page,
